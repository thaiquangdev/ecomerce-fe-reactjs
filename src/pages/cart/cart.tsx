...
import { deleteCartApi, getAllCartsApi } from "../../services/apis/cart";
...
const Cart = () => {
  ...
  const handleDeleteCart = async (pid: string) => {
    const response = await deleteCartApi(pid);
    if (response.data.success === true) {
      toast.success("Delete cart is successfully");
      setCarts((prev) => prev.filter((item) => item.product._id != pid));
    }
  };
